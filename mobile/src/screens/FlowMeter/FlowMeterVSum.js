...
export default class FlowMeterVSum extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data:[],
    ...
    getData=async ()=>{
        ...
        this.socket.on('flowrate',(res)=>{
            ...
            if (currentValueDate >= currentDate && res.FlowRateSum!=null) {
                for (let i=0;i<this.state.data.length;i++){
                    if (this.state.data[i].id===res.objectId){
                        if (res.FlowRateSum<1 && res.FlowRateSum>0){
                            this.state.data[i].value=res.FlowRateSum.toFixed(2);
                        } else if(res.FlowRateSum===0){
                            this.state.data[i].value=res.FlowRateSum;
                        }else {
                            let value=Math.round(res.FlowRateSum).toString();
        ...
        return (
            <View style={styles.body}>
                <View style={styles.listWrapper}>
                    ...
                    <Text style={styles.thirdHeader}>{i18n.t('VSum')}</Text>
                </View>
