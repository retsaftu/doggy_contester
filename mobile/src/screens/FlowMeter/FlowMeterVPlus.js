...
export default class FlowMeterVPlus extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data:[],
    ...
    getData=async ()=>{
        ...
        this.socket.on('flowrate',(res)=>{
            ...
            if (currentValueDate >= currentDate && res.FlowRatePositive!=null) {
                for (let i=0;i<this.state.data.length;i++){
                    if (this.state.data[i].id===res.objectId){
                        if (res.FlowRatePositive<1 && res.FlowRatePositive>0){
                            this.state.data[i].value=res.FlowRatePositive.toFixed(2);
                        } else if(res.FlowRatePositive===0){
                            this.state.data[i].value=res.FlowRatePositive;
                        }else {
                            let value=Math.round(res.FlowRatePositive).toString();
