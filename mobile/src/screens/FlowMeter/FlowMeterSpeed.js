...
export default class FlowMeterSpeed extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data:[],
    ...
    getData=async ()=>{
        ...
        this.socket.on('flowrate',(res)=>{
            ...
            if (currentValueDate >= currentDate && res.FlowRateUzr!=null) {
                for (let i=0;i<this.state.data.length;i++){
                    if (this.state.data[i].id===res.objectId){
                        if (res.FlowRateUzr<1 && res.FlowRateUzr>0){
                            this.state.data[i].value=res.FlowRateUzr.toFixed(2);
                        } else if(res.FlowRateUzr===0){
                            this.state.data[i].value=res.FlowRateUzr;
                        }else {
                            let value=Math.round(res.FlowRateUzr).toString();
        ...
        return (
            <View style={styles.body}>
                <View style={styles.listWrapper}>
                    ...
                    <Text style={styles.thirdHeader}>{i18n.t('FlowSpeed')}</Text>
                </View>
